...
from knuth_shell import shell_sort
from default_shell import shell_sort as default_shell_sort
...
if __name__ == "__main__":
    ...
    print(f"{'n':>7} {'삽입 정렬(s)':>12} {'기본 셸 정렬(s)':>12} {'Knuth 셸 정렬(s)':>12}")
    for n in (20, 40, 1000, 10000):
        a = [random.randrange(n * 10) for _ in range(n)]
        t_ins = measure(insertion_sort, list(a))
        t_shell = measure(default_shell_sort, list(a))
        t_knuth = measure(shell_sort, list(a))
        print(f"{n:>7} {t_ins:>14.6f} {t_shell:>13.6f} {t_knuth:>13.6f}")
